...
public OnPluginStart()
{
	...
	CreateConVar("nativevotes_mapchooser_version", VERSION, "NativeVotes MapChooser version", FCVAR_DONTRECORD|FCVAR_NOTIFY|FCVAR_SPONLY|FCVAR_PLUGIN);
...
public Handler_NV_MapVoteFinished(Handle:menu,
						   num_votes,
						   num_clients,
						   const client_indexes[],
						   const client_votes[],
						   ...
						   const item_votes[])
{
	if (GetConVarBool(g_Cvar_RunOff) && num_items > 1)
	{
		...
		if (winningvotes < required)
		{
			...
			new Float:map2percent = float(item_votes[1])/ float(num_votes) * 100;
...
CreateNextVote()
{
	...
	{
		while (!(GetArraySize(tempMaps) > GetArraySize(g_OldMapList)))
		{
			RemoveFromArray(g_OldMapList, 0);
		}
