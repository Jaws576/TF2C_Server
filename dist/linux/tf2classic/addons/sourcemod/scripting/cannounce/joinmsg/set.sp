		...
		if( client && GetClientAuthId(client, AuthId_Steam2, steamId, sizeof(steamId)) )
		{
			...
			if(KvJumpToKey(hKVCustomJoinMessages, steamId) && !GetConVarInt(g_CvarDisableClientJoinMsg))
			{
				...
				if( strlen(message) > 0)
				{
					...
					ReplyToCommand(client, "[SM] Your custom join message is:")
					PrintFormattedMessageToSelf(output, client);
				}
				else
				{
					ReplyToCommand(client, "[SM] Your custom join message is unset");
				}
